...
(function ($) {
    ...
    $(".btnNav").click(function () {
        if (!$(this)[0].href.includes("#MetricaDashboard")) {
            let menuSelector = '';
            if ($(this)[0].href.includes('http')) { menuSelector = $(this)[0].href.split('#')[1]; }

            else { menuSelector = $(this)[0].href };
            localStorage.setItem('Dashboard', menuSelector);
            $("#MetricaDashboard").css('display', 'none');
        }
        else {
            if (localStorage.getItem("Dashboard").includes('Dashboard')) {
                let menuSelector = '';
                if ($(this)[0].href.includes('http')) { menuSelector = $(this)[0].href.split('#')[1]; }
                ...
                $("#MetricaDashboard").css('display', 'block');
            }
        }
    });
    ...
    function init() {
        if (localStorage.getItem("Dashboard") === null) {
            $("#MetricaDashboard").css('display', 'block');
            setTimeout(function () { $("#btnMetricaDash").click(); }, 1000);
            localStorage.setItem('Dashboard', 'MetricaDashboard');
        }
        initSlimscroll();
        initMetisMenu();
